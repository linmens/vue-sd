        ...
        <x-table :cell-bordered='false' >
          ...
          <tbody>
            <tr v-for="(tr,index) in duizhanglist.list" :key="index">
              <td>{{tr.order_id_jh}}</td>
              <td>{{tr.price_jh}}</td>
              <td>{{tr.price_sjfk_jh}}</td>
              <td>{{tr.price_tk_jh}}</td>
              <td>{{tr.money_ktx_jh}}</td>
              <td>{{tr.sd_yongjin}}</td>
              <td>{{tr.money_ktx_all_jh}}</td>
            </tr>
          </tbody>
        </x-table>
...
methods: {
  ...
  changePage(type){
    ...
    getduizhangApi() {
      ...
      this.$http.post('http://sd.a10store.com/api/user.center.money.duizhang.php',{page:this.page,perPage:(this.wrapperHeight-50)/40}).then(res => {
        console.log(res);
